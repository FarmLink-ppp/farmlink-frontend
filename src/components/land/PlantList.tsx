...
const PlantList: React.FC<PlantListProps> = ({ 
  ...
  return (
    <div className="space-y-4">
      {plants.map((plant) => (
        <Card key={plant.id} className="border-0 bg-white/70 backdrop-blur-sm shadow-lg hover:shadow-xl transition-all duration-300">
          ...
          <CardContent className="space-y-3">
            {plant.description && (
              <p className="text-sm text-farmlink-darkgreen/70">{plant.description}</p>
            )}
